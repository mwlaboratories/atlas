/*
 * laymap layer reporter — broadcast the active ZMK layer to the host.
 *
 * Subscribes to layer state changes and, on each change, sends a raw-HID report
 * via zmk-raw-hid. The laymap desktop overlay reads it from /dev/hidraw* and
 * shows the active layer's keymap.
 *
 * Only the (rare) layer change is sent here — keypresses reach the host as
 * normal HID input, so laymap reads those from the host's evdev stream instead,
...
LOG_MODULE_REGISTER(laymap, CONFIG_ZMK_LOG_LEVEL);

#define LAYMAP_MSG_LAYER 0x4C /* 'L' */
...
ZMK_LISTENER(laymap_layer, layer_state_listener);
ZMK_SUBSCRIPTION(laymap_layer, zmk_layer_state_changed);
